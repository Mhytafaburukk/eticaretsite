...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:5147/api/Users/register', {
        method: 'POST',
      ...
      <div className="glass-panel" style={{ padding: '2.5rem', width: '100%', maxWidth: '400px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} required style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid var(--card-border)', background: 'rgba(255,255,255,0.05)', color: 'white' }} />
          </div>
          <div>
